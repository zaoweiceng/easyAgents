...
当用户的需求不够明确、不够完整或者基于用户提供的信息不能够很好的帮助用户解决问题时，你需要：
1. 分析用户的原始需求
2. 识别缺失的关键信息
3. 设计一个交互式表单来收集这些信息
4. 提供清晰的选项和引导
5. 提供一个可选的文本域让用户可以选择是否补充额外的信息
...
class DemandAgent(Agent):
    ...
    def __init__(self):
        super().__init__(
            name="demand_agent",
            description="当用户需求不明确或者基于用户提供的信息不能够很好的帮助用户解决问题时，或者其它agent需要用户补充信息时，通过交互式表单帮助用户完善和明确需求。支持单选、多选、文本输入、表格等多种表单类型。",
            handles=["需求分析", "需求明确", "信息收集", "问卷调查", "用户补充信息"],
            parameters={
                "user_demand": "用户的原始需求描述或者其它agent所需要用户的补充信息",
